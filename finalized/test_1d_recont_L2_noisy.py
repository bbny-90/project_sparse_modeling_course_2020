...
if 1:
    ...
    plt.plot(x[meas_indx], data[meas_indx, target_field_id], 'rx')
    plt.show()
    plt.close()

y = data[meas_indx, target_field_id]
y_noise = np.random.randn(len(y)) * 0.
# y_noise[2] = y_noise[2] * 0.1
y += y_noise


w = cvx.Variable(train_data.shape[1])
loss = cvx.sum_squares(train_data @ w - y )/2
...
y_recon = (data[:, 0:train_indx] @ w.value+mean_data)*std_data
...
plt.tight_layout()
plt.show()
print(np.linalg.norm(y_exact - y_recon))